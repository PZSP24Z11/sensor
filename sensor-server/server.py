...
class Server:
    ...
    def start(self) -> None:
        ...
        self._sock = ServerContext(self.srv_conf).wrap_socket(socket.socket(socket.AF_INET, self.proto))
    ...
    def run(self, conn_handler: Callable[[TLSWrappedSocket], None]) -> NoReturn:
        if not self._sock:
...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument("--tls", dest="proto", action="store_const", const=socket.SOCK_STREAM)
    group.add_argument("--dtls", dest="proto", action="store_const", const=socket.SOCK_DGRAM)
...
def main(args: argparse.Namespace) -> NoReturn:
    ...
    certa = "./../cert/server.der"

    with open(certa, "rb") as f:
        cert2 = x509.load_der_x509_certificate(f.read())

    pk = mbedtls.tls.PrivateKey.from_file("./../cert/server.key", "root")

    if args.proto is socket.SOCK_DGRAM:
        conf = DTLSConfiguration(
            pre_shared_key_store=args.psk_store, validate_certificates=True, certificate_chain=((cert2), pk)
        )
    else:
