...
def main(args: argparse.Namespace) -> None:
    ...
    with open("./../cert/ca.der", "rb") as f:
        cert = x509.load_der_x509_certificate(f.read())

    if args.proto is socket.SOCK_STREAM:
        conf = TLSConfiguration(pre_shared_key=args.psk, validate_certificates=False)
    elif args.proto is socket.SOCK_DGRAM:
        conf = DTLSConfiguration(pre_shared_key=args.psk, validate_certificates=True, certificate_chain=((cert), None))
    else:
        raise NotImplementedError(args.proto)

    with Client(conf, args.proto, (args.address, args.port), args.server_name) as cli:
    ...
    print(f"Recieved: {received.decode("utf-8")}")
